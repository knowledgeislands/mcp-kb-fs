...
Claude calls [`kb_note_write`](#kb_note_write) with the markdown content and `create_dirs: true` (the default). The path goes through both the lexical and `realpath` safety checks before any byte is written. `kb_note_write` is annotated `DESTRUCTIVE` (it can overwrite an existing note), so `MCP_KI_KB_FS_ACCESS_LEVEL=destructive` is required for it to register.
